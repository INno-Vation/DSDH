...
def find_top_rpn_proposals(
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        keep = batched_nms(boxes.tensor, scores_per_img, lvl, nms_thresh)  # nms_thresh
        ...
        # num = keep.shape[0]
        # choice = min(1000, num)
        # index = torch.LongTensor(random.sample(range(num), choice)).to(device='cuda:0')
        # keep = torch.index_select(keep, 0, index)
